"""Synchrotron facility safety flagging.

Pure-Python, side-effect-free. The chat layer asks `detect_safety_topic` for
every user query; if anything matches, the UI keeps the answer grounded in
retrieved evidence and directs the user to beamline staff or emergency contacts.
...
# Chroma cosine distance >0.55 usually means the top hit is a stretch.
LOW_CONFIDENCE_DISTANCE = 0.55


# Facility emergency contacts. Update this list — not the UI — when numbers change.
